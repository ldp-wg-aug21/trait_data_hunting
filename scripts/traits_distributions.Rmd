...
trait_stats_discrete<-function(traitname){
...
fish<-fread("../data-clean/fishbase_traits-for-comparison.csv")
...
colnames(troph.plot)<-c("TrophicLevel","Group","lpi","N")
troph.plot$TrophicLevel<-factor(troph.plot$TrophicLevel,levels=c("herbivore","omnivore","carnivore"))

ggplot(troph.plot,aes(x=lpi,y=N,fill=TrophicLevel))+
  geom_col(position="fill")+
  facet_wrap(vars(Group),nrow=2)+
  scale_fill_manual(values=wes_palette("Royal2")[3:5])+
  theme_classic()+
  theme(legend.title=element_blank())



trait_stats_discrete("TrophicLevel")
