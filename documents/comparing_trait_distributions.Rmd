...
The `lpi` column indicates whether the species is represented within the C-LPI timeseries dataset. Note, however, that the species that do occur in C-LPI are duplicated in this dataset (i.e., then occur once with `lpi = "C-LPI"` and once with `lpi = "C-Vertebrates"`).
...
Let's split the dataset in two, recreating `vertslpi` and `vertswild` that are used to generate this combined dataset in the data prepartion script (see above).
...
Next, we need to find the difference between these datasets. That is, find the Canadian species that **aren't** represented in the C-LPI. We will then use the two lists create a different version of the dataset that includes a single row per species, with a column indicating whether they occur in the C-LPI subset (`clpi = binary: yes, no`).
...
Here I am using the `dabestr` package (see [vignette](https://cran.r-project.org/web/packages/dabestr/vignettes/using-dabestr.html)) to run the bootstrapping (n = 5,000 samples) and produce Gardman-Altman estimation plots of the results. The plots show the raw data (species-specific trait measurements), the mean/median for each group, and the mean/mean difference between groups and corresponding confidence interval.

**Note**: I've used a similar approach in the past and have found it to be a really intuitive way of bring some statistical insight to the data. This, combined with a detailed description of the nuanced ways the distributions differ, will I think be quite convincing! 
